...
object CollisionEngine:
  ...
  def computeCollision[A](a1: A, a2: A)(using Intersection[A]): Boolean =
   ...
   */
  def computeImpact[A](a1: A, a2: A)(using Intersection[A])(using Impact[A]): A =
    if computeCollision(a1, a2) then summon[Impact[A]].impact(a1, a2) else a1
  ...
  def impactMany[A](a: A, others: Seq[A])(using Intersection[A])(using Impact[A]): A =
    others.foldLeft(a)((acc, a1) => computeImpact(acc, a1))