...
object EntityReferenceDetectors:
  given SphereOfInfluenceDetector: EntityReferenceDetector with
    ...
    //should already have a list where element's distance is less than roi
    ...
    /*@tailrec
    private def halo(list: List[(CelestialBody, Double, Double)], ref: Option[(CelestialBody, Double, Double)] = Option.empty): Option[CelestialBody] = ref match
      case r if r.isEmpty && list.nonEmpty => halo(list.tail, Option(list.head._1, list.head._2, list.head._3))
      case r if r.nonEmpty && list.nonEmpty => list.head match
        case (b, d, r) if d < ref.get._2 && ref.get._3 > r => list.tail match
          case l if l.nonEmpty => halo(list.tail, Option(b, d, r))
          case _ => Option(ref.get._1)
        case _ => halo(list.tail, ref)
      case r if r.isEmpty && list.isEmpty => Option.empty
      case _ => Option(ref.get._1)
*/